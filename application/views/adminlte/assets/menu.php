...
<ul class="sidebar-menu" data-widget="tree">
    ...
    <li<?=$row->child ? ' class="treeview"' : '';?>>
        ...
        <?php show_sub_menu($row, $login_data['user']->usertype); ?>
    </li>
    <?php endforeach; ?>
</ul>
...
var ar_folders = ['rawdata','report'];

var url = window.location;
var ar_url = String(url).split("/");
var ar_comp = String(url).split("/");
// console.log('<?=site_url().(substr(site_url(),-1)!='/'?'/':''); ?>')
var comp_length = <?=count(explode("/",site_url().(substr(site_url(),-1)!='/'?'/':''))); ?>;
...
function show_sub_menu($row_menuitem, $usertype){
...
?>
	<li<?=$row->child ? ' class="treeview"' : '';?>>
        ...
        <?php show_sub_menu($row, $usertype); ?>
	</li>
	<?php
	endforeach;
	?>
