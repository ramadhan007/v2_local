...
class DeviceModel extends CI_Model {
	...
	function get_paged_list($limit = 10, $offset = 0, $filters=array()){
		...
		$sql = "
			SELECT 	a.*,
					if(isnull(a.status_time),'',TIME_FORMAT(TIMEDIFF(NOW(),a.status_time),'%H:%i:%s')) as status_age,
					...
					fn_get_list_item_short('device_status',a.status_final) as status_text,
					fn_get_list_item_icon('device_status',a.status_final) as status_icon,
					fn_get_list_item_class('device_status',a.status_final) as status_class,
					fn_get_list_item_short('platform',platform_name) as platform_text
			FROM 	$this->table as a
					INNER JOIN tb_location as b ON a.location_id = b.id
					INNER JOIN tb_operator AS c ON a.operator_id = c.id
			".($where ? "WHERE $where" : "")."
