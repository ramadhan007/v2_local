...
class KpiTrend extends Admin_Controller {

	function __construct(){
		parent::__construct(__FILE__, 'KPI trends', 'report/kpitrend');
	...
	function _show(){	
		...
		// load data
		// $rows = $this->model->get_paged_list($filter_monitor_date_start, $filter_monitor_date_end, $filter_location_id, $filter_operator_id, $filter_journey_detail_id)->result();
		$rows = array();
		$data['array_data'] = $this->_rows2array($rows);
	...
	function _rows2array($rows){
		...
		if(count($rows)){
			...
			$array[] = array('label', 'response_time_avg', 'nineth_percentile', 'response_time_mean', 'ux_index',
				'it_availability', 'eu_availability');
