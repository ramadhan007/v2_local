...
class ExceptionData extends Admin_Controller {

	function __construct(){
		parent::__construct(__FILE__, 'exception data', 'report/exceptiondata');
	...
	function _show($offset = 0){	
		...
		$config['base_url'] = site_url($this->controller.'/index/');
 		$config['total_rows'] = $this->model->count_all($filter_monitor_date_start, $filter_monitor_date_end, $filter_location_id, $filter_operator_id);
	...
	function listcontent($param='', $offset=''){	
		...
		elseif($param=='paginfo'){
			$numrows = count($this->model->get_paged_count($this->limit, $offset, $filter_monitor_date_start, $filter_monitor_date_end, $filter_location_id, $filter_operator_id)->result());
			$totalrows = $this->model->count_all($filter_monitor_date_start, $filter_monitor_date_end, $filter_location_id, $filter_operator_id);
			if($numrows){
	...
	function checkdownloadxls($location_ids, $operator_ids, $export_date_start, $export_date_end){
		...
		$ar_query[0] = 	"SELECT COUNT(a.id)
			...
			WHERE	a.status AND f.id IN ($location_ids) AND g.`id` IN ($operator_ids)
	...
	function downloadxls($location_ids, $operator_ids, $export_date_start, $export_date_end){
		...
		$ar_query[0] = 	"
			SELECT	a.id, f.name AS 'Location', g.name AS 'Operator',
					REPLACE(REPLACE(CONCAT(d.name,'_',f.name,'_',g.name),' ','_'),'/','_') AS 'Application',
					c.name AS 'Page Name',
					b.`monitor_date`, DATE_FORMAT(a.monitor_datetime,'%H:%i:%S') AS 'Start Time',
					DATE_FORMAT(h.`recover_datetime`, '%H:%i:%S') AS 'End Time', TIMEDIFF(h.`recover_datetime`, a.monitor_datetime) AS 'Error Duration',
					IF(a.scheduled,'Scheduled','Unscheduled') AS 'Error Type',
					a.message as 'Error Description'
			FROM 	`tb_monitor_journey_detail` AS a 
					INNER JOIN `tb_monitor_journey` AS b ON (a.device_id = b.device_id AND a.monitor_date = b.monitor_date AND a.`monitor_journey_id` = b.`id`) 
					INNER JOIN `tb_journey_detail` AS c ON a.`journey_detail_id` = c.`id` 
					INNER JOIN `tb_journey` AS d ON b.`journey_id` = d.`id` 
