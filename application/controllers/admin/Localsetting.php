...
class Localsetting extends Admin_Controller {

	function __construct(){
		parent::__construct(__FILE__, 'local setting', 'localsetting');
	}
	...
	function _set_rules($editmode=FALSE){
		if($editmode){
			...
			$this->form_validation->set_rules('name','Name','trim|required|is_unique[tb_setting.name]');
			$this->form_validation->set_rules('value','Value','trim|required');
			$this->form_validation->set_rules('unit','Unit','trim');
		}
	}
	...
	function _set_rules_name($changed=FALSE){
		if($changed){
			$this->form_validation->set_rules('name','Nama','trim|required|is_unique[tb_setting.name]');
		}else{
