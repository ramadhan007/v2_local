...
class Device extends Admin_Controller {
	...
	function _set_rules($editmode=FALSE){
		...
		$this->form_validation->set_rules('udid','Device SN','trim');
		$this->form_validation->set_rules('platform_name','Platform Name','trim');
		$this->form_validation->set_rules('platform_version','Platform Version','trim');
		...
		$this->form_validation->set_rules('application','Application','trim|required');
	...
	function _get_html($row=array())
	{
		...
		$html['platform_name'] = html_select('platform', $array_data, set_value('platform_name', ($row ? $row->platform_name : '')),'All');
	...
	function _get_post_data($editmode=FALSE){
		$row = array(
				...
				'platform_version' => $this->input->post('platform_version'),
