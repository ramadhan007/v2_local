...
class Journey extends Admin_Controller {
	...
	function __construct(){
		parent::__construct(__FILE__, 'journey', 'journey');
	}
	...
	function _set_rules($editmode=FALSE){
		if($editmode){
			...
			$this->form_validation->set_rules('name','Name','trim|required|is_unique[tb_journey.name]');
		}
		$this->form_validation->set_rules('activity_name','Activity Name','trim');
	...
	function _get_html($row=array())
	{
		...
		//application
		$array_data = get_list_item('journey_type');
		$html['type'] = html_select('type', $array_data, set_value('type', ($row ? $row->type : '')), 'Select Type');
