...
class Listcat extends Admin_Controller {

	function __construct(){
		parent::__construct(__FILE__, 'kategori list', 'listcat');
	}
	...
	function updateData(){
		...
		if($str!=$this->model->get_field_by_id($id, 'tag')){
			$this->_set_rules_tag(TRUE);
		}
		else{
			$this->_set_rules_tag(FALSE);
		}
	...
	function _set_rules($editmode=FALSE){
		if($editmode){
			...
			$this->form_validation->set_rules('name','Nama','trim|required|is_unique[tb_list_cat.name]');
			// $this->form_validation->set_rules('tag','Tag','trim|required|is_unique[tb_list_cat.tag]');
			$this->form_validation->set_rules('tag','Tag','trim');
		}
		$this->form_validation->set_rules('type','Tipe Nilai','trim|required');
	}
	...
	function _set_rules_tag($changed=FALSE){
		$this->form_validation->set_rules('tag','Tag','trim');
		if($changed){
			// $this->form_validation->set_rules('tag','Tag','trim|required|is_unique[tb_list_cat.tag]');
		}else{
	...
	function _get_html($row=array())
	{
		...
		//type
		$array_data = array(
				'0' => array('value' => 'text', 'text' => 'Text'),
				'1' => array('value' => 'number', 'text' => 'Number'),
			);
		$html['type'] = html_select('type', $array_data, set_value('type', ($row ? $row->type : 'text')));
	...
	function _get_post_data(){
		...
		$tag = $tag ? $tag : str_replace(" ","_",strtolower($name));
