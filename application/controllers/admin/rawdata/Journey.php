...
class Journey extends Admin_Controller {
	...
	function __construct(){
		parent::__construct(__FILE__, 'Journey Raw Data', 'rawdata/journey');
	...
	function downloadxls($location_ids, $operator_ids, $export_date_start, $export_date_end){
		...
		$namafile = "raw_journey_".($export_date_start==$export_date_end ? $export_date_start : $export_date_start."_".$export_date_end);
		
		$ar_judulsheet[0] = "Sheet1";
		$ar_query[0] = 	"SELECT	b.`id`, f.name AS location_name, g.name AS operator_name, d.name AS journey_name, max(if(a.network_type='3G', mod(a.`cellid`,65536), a.`cellid`)) as cellid, b.`location_lat`, b.`location_lng`,
					MAX(a.`network_type`) AS network_type, ROUND(AVG(IF(a.signal_level<99,a.signal_level,NULL)),0) AS signal_level, 
					ROUND(AVG(IF(a.signal_quality<99,a.signal_quality,NULL)),0) AS signal_quality,
					ROUND(AVG(IF(a.ber<99,a.ber,NULL)),0) AS ber, ROUND(SUM(a.`response_time`),3) AS response_time, b.`monitor_datetime`,
					(case max(a.status)
					...
					(case max(a.scheduled) when 1 then 'Scheduled' else 'Unscheduled' end) as error_type,
					MAX(a.message) AS message,
					IF(MAX(a.status)>0,fn_get_journey_detail_name(MAX(a.journey_detail_id)),'') AS 'error_on',
			...
			GROUP 	BY b.id, f.name, g.name, d.name, b.`location_lat`, b.`location_lng`
			ORDER 	BY b.monitor_datetime DESC";
	...
	function qrydownload($location_ids, $operator_ids, $export_date_start, $export_date_end){
		...
		$ar_judulsheet[0] = "Sheet1";
		$ar_query[0] = 	"SELECT	b.`id`, f.name AS location_name, g.name AS operator_name, d.name AS journey_name, max(if(a.network_type='3G', mod(a.`cellid`,65536), a.`cellid`)) as cellid, b.`location_lat`, b.`location_lng`,
					MAX(a.`network_type`) AS network_type, MAX(a.`signal_level`) AS signal_level, ROUND(SUM(a.`response_time`),3) AS response_time, b.`monitor_datetime`,
					(case max(a.status)
						when '3' then 'Wrong PIN'
						when '2' then 'Miss Page'
						when '1' then 'General Error'
