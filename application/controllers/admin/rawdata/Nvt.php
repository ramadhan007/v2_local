...
class Nvt extends Admin_Controller {

	var $num_links = 10;
	
	function __construct(){
		parent::__construct(__FILE__, 'NVT Raw Data', 'rawdata/nvt');
	...
	function downloadxls($location_ids, $operator_ids, $export_date_start, $export_date_end){
		...
		$namafile = "raw_nvt_".($export_date_start==$export_date_end ? $export_date_start : $export_date_start."_".$export_date_end);
		
		$ar_judulsheet[0] = "Sheet1";
		$ar_query[0] = 	"SELECT	e.id AS device_id, f.name AS location_name, g.name AS operator_name, d.name AS journey_name,
					if(a.network_type='3G', mod(a.`cellid`,65536), a.`cellid`) as cellid, b.`location_lat`, b.`location_lng`, a.`network_type`, a.`signal_level`, a.signal_level, a.ber, a.`response_time`, a.monitor_datetime,
					(case a.status
