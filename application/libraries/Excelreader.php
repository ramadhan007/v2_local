...
//end of changes by mmp
	...
	public function getResult($colname='',$casecheck=false)
	{
		if($colname){
			$colname = is_array($colname) ? $colname : array($colname);
		}else{
			$colname = array();
		}
		if($colname && !$casecheck)
		{
			for($i=1; $i<=$this->sheets[0]['numCols']; $i++)
			{
				$this->sheets[0]['cells'][1][$i] = strtolower($this->sheets[0]['cells'][1][$i]);
			}
			foreach($colname as $key => $val)
			{
				$colname[$key] = strtolower($val);
			}
		}
		$rows = array();
		for($i=0; $i<$this->sheets[0]['numRows']; $i++)
		{
			$k = 0;
			for($j=0; $j<$this->sheets[0]['numCols']; $j++)
			{
				if($colname)
				{
					if(in_array($this->sheets[0]['cells'][1][$j+1],$colname))
					{
						$rows[$i][$k++] = isset($this->sheets[0]['cells'][$i+1][$j+1]) ? $this->sheets[0]['cells'][$i+1][$j+1] : "";
					}
				}
				else
				{
					$rows[$i][$j] = isset($this->sheets[0]['cells'][$i+1][$j+1]) ? $this->sheets[0]['cells'][$i+1][$j+1] : "";
				}
			}
		}
		return $rows;
	...
	public function checkColumn($colname, $casecheck = false, $rows=array())
	{
		$rows = count($rows) ? $rows : $this->getResult();
		if(count($rows))
		{
			$row = &$rows[0];
			if(!$casecheck)
			...
			if(is_array($colname))
			{
				foreach($colname as $key => $val)
				{
					if(!in_array($val,$row)) return false;
				}
			}
			else
			{
				if(!in_array($colname,$row)) return false;
			}
			return true;
		}
		else
	...
	public function getColumnIndex($colname, $casecheck = false, $rows=array())
	{
		$rows = count($rows) ? $rows : $this->getResult();
		if(count($rows))
		{
	...
	public function fixDate($strdate)
	{
		if($strdate){
			$ar_date = explode('/',$strdate);
			return $this->_dateAdd($ar_date[2].'-'.$ar_date[1].'-'.$ar_date[0],0,0,-1,0,0,0);
		}else{
			return mktime(0,0,0,0,0,0);
		}
	}
	
	function _dateAdd($givendate,$yr=0,$mth=0,$day=0,$hour=0,$min=0,$sec=0)
	{
