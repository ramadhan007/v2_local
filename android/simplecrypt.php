...
class SimpleCrypt{
	// private $strkode = "_6%_g#q77F-dWCV4jS9-E96pu=@uke^#C9Qa&MgxfFYFmLv5gjRZr4axX9m@Dtrmhgr!C-My-Wzm!6ULdr*yE^8rKHD8Z-*?u4uv";
	private $strkode = "";
	...
	private function encrypt1($strtext, $strkode)
	{
		...
		for($i=1;$i<=$a;$i++)
		{
			...
			$arnumhsl[$i-1] = $arasctext[$i-1]*3 + $arasckode[$j-1];
		}
		for($i=1;$i<=$a;$i++)
		{
			$arnumhsl[$i-1] = $arnumhsl[$i-1] + 99;
		}
		for($i=1;$i<=$a;$i++)
		{
	...
	private function decrypt1($strtext, $strkode)
	{
		...
		for($i=0; $i<(strlen($strtext)/2); $i++){
			$artemp[$i] = substr($strtext,$i*2,2);
		}
		$a = count($artemp);
		for($i=1;$i<=$a;$i++)
		{
		...
		for($i=1;$i<=$a;$i++)
		{
			...
			$arasctext[$i-1] = ($arnumhsl[$i-1] - $arasckode[$j-1])/3;
		}
		$strtext = "";
		for($i=1;$i<=$a;$i++)
		{
