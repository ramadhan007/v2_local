...
if(date('H:i', time())!='00:00'){
	...
	$sql = "select * from tb_trigger_device order by id asc limit 0,1";
	...
	if($result['num_rows']>0){
		$rows = $result['rows'];
		foreach($rows as $row){
			// echo "https://realdataidapm.com/dashboard/php/trigger/updatedevice.php?app=4d74cx4fz&device_id=".$row['device_id']."&status=".$row['status']; exit();
	...
	$sql = "select * from tb_trigger_journey order by id asc limit 0,1";
