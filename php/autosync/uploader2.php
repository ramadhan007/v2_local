...
if(date('H:i', time())!='00:00'){
	...
	// get setting
	$sql = "select value from tb_setting where name = 'screenshot_path_rel'";
	$result = $db->db_query($sql);
	
	if($result['num_rows']>0){
		$screenshot_path_rel = $result["rows"][0]["value"];
	}else{
		$screenshot_path_rel = "userfiles/screenshot/monitor_journey_detail";
	}
	
	// get setting
	$sql = "select value from tb_setting where name = 'screenshot_ext'";
	$result = $db->db_query($sql);
	
	if($result['num_rows']>0){
		$screenshot_ext = $result["rows"][0]["value"];
	}else{
		$screenshot_ext = "png";
	}
	...
	while($ada_update){
		...
		$sql = "select * from tb_log_update".($device_id ? " where device_id = '$device_id'" : "")." order by id asc limit 0,1";
		$result = $db->db_query($sql);
